...
import { users, habits, entries, type NewUser, type NewHabit, tags, habitTags } from '../../src/db/schema.ts'
...
export const createTestUser = async (userData: Partial<NewUser> = {}) => {
  ...
  const [user] = await db.insert(users).values({ ...defaultData, password: hashedPassword }).returning();
